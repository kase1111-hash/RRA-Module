...
class AssetTokenizer:
    ...
    def __init__(
        ...
        compliance_verifier: Optional[str] = None
    ):
    ...
    def create_tokenization_request(
        ...
        metadata.ownership_records.append(OwnershipRecord(
    ...
    def add_document(
        ...
        if request.status not in [TokenizationStatus.DRAFT, TokenizationStatus.PENDING_VERIFICATION]:
    ...
    def get_assets_by_type(self, asset_type: AssetType) -> List[TokenizedAsset]:
        ...
        return [
            ...
            if asset.metadata.asset_type == asset_type
        ]
    ...
    def set_fractionalized(
        self,
        token_id: int,
        fractional_contract: str
    ) -> TokenizedAsset:
    ...
    def generate_token_metadata(self, token_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "image": f"ipfs://{metadata.legal_description_hash}" if metadata.legal_description_hash else None,
            ...
            "attributes": [
                ...
                {"trait_type": "Registration Authority", "value": metadata.registration_authority.value},
                {"trait_type": "Origin Jurisdiction", "value": metadata.origin_jurisdiction},
                {"trait_type": "Registration Date", "value": metadata.registration_date.isoformat()},
                {"trait_type": "Expiration Date", "value": metadata.expiration_date.isoformat() if metadata.expiration_date else "Perpetual"},
                {"trait_type": "Physical Asset", "value": "Yes" if metadata.is_physical_asset else "No"},
                {"trait_type": "Compliance Status", "value": asset.compliance_status},
                {"trait_type": "Transfer Restricted", "value": "Yes" if asset.transfer_restricted else "No"},
                {"trait_type": "Fractionalized", "value": "Yes" if asset.is_fractionalized else "No"},
            ],
            "properties": {
                "royalty_basis_points": asset.royalty_basis_points,
                "minimum_price": str(asset.minimum_price),
                "current_valuation": str(asset.current_valuation) if asset.current_valuation else None,
