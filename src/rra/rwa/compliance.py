...
class RWAComplianceChecker:
    ...
    def register_participant(
        self, address: str, jurisdictions: Optional[List[str]] = None
    ) -> ParticipantProfile:
    ...
    def update_accreditation(
        self, address: str, accreditation_type: AccreditationType, expires_in_days: int = 365
    ) -> ParticipantProfile:
    ...
    def update_aml_status(self, address: str, cleared: bool) -> ParticipantProfile:
    ...
    def update_sanctions_status(self, address: str, cleared: bool) -> ParticipantProfile:
    ...
    def update_pep_status(
        self, address: str, is_pep: bool, pep_cleared: bool = False
    ) -> ParticipantProfile:
    ...
    def set_jurisdiction_rules(self, country_code: str, rules: JurisdictionRules):
    ...
    def check_tokenization_compliance(
        ...
        if not owner:
            ...
            checks.append(
                self._create_check(
                    "owner_registration", False, "Owner must be registered for compliance tracking"
                )
            )
        else:
            checks.append(self._create_check("owner_registration", True, "Owner is registered"))
        ...
        for jurisdiction in target_jurisdictions:
            ...
            if not jurisdiction_check.passed:
                if jurisdiction == origin_jurisdiction:
                    ...
                    warnings.append(
                        f"Target jurisdiction {jurisdiction}: {jurisdiction_check.message}"
                    )
    ...
    def check_transfer_compliance(
        ...
        if not sender:
            ...
            checks.append(
                self._create_check("sender_registration", False, "Sender must be registered")
            )
        else:
            checks.append(self._create_check("sender_registration", True, "Sender is registered"))
        ...
        if not recipient:
            ...
            checks.append(
                self._create_check("recipient_registration", False, "Recipient must be registered")
            )
        else:
            checks.append(
                self._create_check("recipient_registration", True, "Recipient is registered")
            )
                ...
                for jurisdiction in recipient.jurisdictions:
                    ...
                    if not jurisdiction_check.passed:
                        blocking_issues.append(
                            f"Recipient jurisdiction {jurisdiction}: {jurisdiction_check.message}"
                        )
        ...
        if transfer_value < minimum_price:
            ...
            checks.append(
                self._create_check(
                    "minimum_price", False, f"Transfer value must be at least {minimum_price}"
                )
            )
        else:
            checks.append(self._create_check("minimum_price", True, "Transfer value meets minimum"))

        overall_passed = len(blocking_issues) == 0

        return self._create_report(
            "transfer", "asset", str(token_id), overall_passed, checks, blocking_issues, warnings
        )

    def check_participant_eligibility(
        self, address: str, jurisdiction: str, asset_type: str
    ) -> ComplianceReport:
        ...
        if not participant:
            ...
            checks.append(
                self._create_check("registration", False, "Participant must be registered")
            )
        else:
            # KYC check
            ...
            if rules:
                if rules.is_restricted:
                    ...
                    checks.append(
                        self._create_check(
                            "jurisdiction", True, f"Jurisdiction {jurisdiction} is allowed"
                        )
                    )

                    # Accreditation check
                    if rules.requires_accreditation:
                        accred_check = self._check_accreditation(
                            participant, rules.minimum_accreditation
                        )
                        checks.append(accred_check)
                        if not accred_check.passed:
        ...
        return self._create_report(
            "eligibility", "participant", address, overall_passed, checks, blocking_issues, warnings
        )
    ...
    def _check_kyc(self, participant: ParticipantProfile) -> ComplianceCheck:
        ...
        if participant.kyc_status != KYCStatus.VERIFIED:
            return self._create_check(
                "kyc", False, f"KYC not verified (status: {participant.kyc_status.value})"
            )

        if participant.kyc_expires_at and participant.kyc_expires_at < datetime.utcnow():
            return self._create_check("kyc", False, "KYC has expired")

        return self._create_check("kyc", True, "KYC verified and current")

    def _check_aml(self, participant: ParticipantProfile) -> ComplianceCheck:
        """Check AML clearance."""
        if not participant.aml_cleared:
            return self._create_check("aml", False, "AML check not passed")

        return self._create_check("aml", True, "AML cleared")
    ...
    def _check_sanctions(self, participant: ParticipantProfile) -> ComplianceCheck:
        ...
        if not participant.sanctions_cleared:
            return self._create_check("sanctions", False, "Sanctions check not passed")
        ...
        if participant.sanctions_checked_at:
            if datetime.utcnow() - participant.sanctions_checked_at > timedelta(days=30):
                return self._create_check(
                    "sanctions", False, "Sanctions check is stale (> 30 days old)"
                )

        return self._create_check("sanctions", True, "Sanctions cleared")

    def _check_pep(self, participant: ParticipantProfile) -> ComplianceCheck:
        """Check PEP status."""
        if participant.is_pep and not participant.pep_cleared:
            return self._create_check("pep", False, "PEP status requires additional clearance")

        return self._create_check("pep", True, "PEP check passed")

    def _check_accreditation(
        self, participant: ParticipantProfile, minimum: AccreditationType
    ) -> ComplianceCheck:
        ...
        # Check expiration
        if (
            participant.accreditation_expires_at
            and participant.accreditation_expires_at < datetime.utcnow()
        ):
            return self._create_check("accreditation", False, "Accreditation has expired")

        return self._create_check(
            "accreditation", True, f"Accreditation verified: {participant.accreditation_type.value}"
        )
    ...
    def _create_check(
        self, check_type: str, passed: bool, message: str, details: Optional[Dict] = None
    ) -> ComplianceCheck:
