# FAQ: Challenges and Resilience Analysis
...
Consensus involves explicit linguistic negotiation/clarification protocols among LLM-powered nodes (e.g., "Does this entry mean X? Rephrase to confirm."). This turns ambiguity into a feature: ongoing discourse resolves it transparently, preserving the full auditable "conversation" on-chain. Unlike symbolic systems where ambiguity hides in code, here it's surfaced and negotiated in readable form.

### Scalability mitigated by focus
This isn't for high-frequency trading or micro-transactions (where binary wins). It's optimized for human-scale, high-value applications (personal finance, taxes, agent economies, governance) where readability/auditability trumps raw speed. Techniques like summarization, compression, or hybrid layers (prose + hashes) can help; off-chain storage with on-chain commitments is feasible.
